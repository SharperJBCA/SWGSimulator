...
beam_type_map = {'airy':'ModAiryBeam','gauss':'GaussianBeam'}
def main(parser):
    print('INFO: Loading beams')
    ...
    print('INFO: Creating common data')
    common_data = {}
    for telescope in parser['SimObs']['telescopes']:
        ...
        for beam_type, beam_data in beams[telescope].items():
            ...
            common_data[telescope][beam_type_map[beam_type]] = {'FWHM':[beam_data.FWHM, {'UNIT':'degree'.encode()}],
                                                                'Frequency': [frequencies ,{'UNIT':'MHz'.encode()}],
                                                                'BeamModel': [beam_data.model[:,good],{}],
                                                                'Colatitude': [beam_data.theta[good],{'UNIT':'degree'.encode()}],
                                                                'Hits':[hits[mask]*parser[telescope]['dishes']*parser[telescope]['repeats'],
                                                                        {'UNIT':'seconds'.encode()}]}
    ...
    output = h5py.File('{}/swg_fgchallenge2020_withHI:{}_withFGs:{}_withNoise:{}_{}_{}.hd5'.format(parser['SimObs']['output_dir'],
                                                                                                   parser['SimObs']['include_hi'],
                                                                                                   parser['SimObs']['include_fgs'],
                                                                                                   parser['SimObs']['noise_mode'],
                                                                                                   parser['SimObs']['noise_mode'],
                                                                                                   parser['SimObs']['version']),'a')
    print('INFO: Creating maps')
    ...
    for (skysim, telescope, beamtype) in zip(tqdm(x.flatten()), y.flatten(), z.flatten()):
        ...
        if  parser['SimObs']['include_fgs']:
            if 'Total' in sky_maps[skysim]:
                fg_maps = sky_maps[skysim]['Total'][:,...]
            else:
                fg_maps = sky_maps[skysim]['Model_Sky'][:,...]
            if 'scale_fgs' in parser['SimObs']:
                monopole = sky_maps[skysim]['Monopole'][...]
                fg_maps = (fg_maps[...] - monopole[:,None])*parser['SimObs']['scale_fgs'] + monopole[:,None]
        else:
            monopole = sky_maps[skysim]['Monopole'][...]
            fg_maps = monopole[:,None] + np.zeros((1,npixels))


        maps = fg_maps + hi_maps[...]
        grp  = HDF5Tools.get_group('{}/{}/{}'.format(skysim, telescope, beam_type_map[beamtype]), output)
        ...
        for i in tqdm(range(frequencies.size)):
            ...
            if parser['SimObs']['noise_mode'] == 'white':
                m += NoiseModels.GetNoise(m,common_data[telescope][beam_type_map[beamtype]]['Hits'][0])
            ...
            dset[i,:] = m
        HDF5Tools.create_ancillary(grp,common_data, skysim, telescope, beam_type_map[beamtype])
    output.close()
